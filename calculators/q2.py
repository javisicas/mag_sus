...
class Mag_sus_occ_2_orb_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        ...
        kron = data_K.Q2.kron
        # kron = 1 - data_K.Q2.smooth_cutoff
...
class Mag_sus_occ_2_orb_wang_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        super().__init__(data_K, **parameters)
        O = data_K.Q2.berry_curvature_wang_non_abelian
        M = data_K.Q2.magnetic_dipole_orb
        kron = data_K.Q2.kron
...
class Mag_sus_occ_2_orb_wang(DynamicCalculator):
    def __init__(self, spin=True, **kwargs):
        super().__init__(**kwargs)
        self.kwargs_formula.update(dict(spin=spin))
        self.Formula = Mag_sus_occ_2_orb_wang_formula
        self.constant_factor =  factors.factor_cell_volume_to_m
        self.transformTR = transform_ident
        self.transformInv = transform_ident
        self.sign = 1
...
class Mag_sus_occ_2_orb_wang_2_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        super().__init__(data_K, **parameters)
        O = data_K.Q2.berry_curvature_wang_non_abelian_2
        M = data_K.Q2.magnetic_dipole_orb
        kron = data_K.Q2.kron
...
class Mag_sus_occ_2_orb_wang_2(DynamicCalculator):
    def __init__(self, spin=True, **kwargs):
        super().__init__(**kwargs)
        self.kwargs_formula.update(dict(spin=spin))
        self.Formula = Mag_sus_occ_2_orb_wang_2_formula
        self.constant_factor =  factors.factor_cell_volume_to_m
        self.transformTR = transform_ident
        self.transformInv = transform_ident
        self.sign = 1
...
class Mag_sus_inter_formula(Formula):
    def __init__(self, data_K, spin=True, Efermi=0, scissor=0, **parameters):
        super().__init__(data_K, **parameters)
...
class Mag_sus_inter(DynamicCalculator):
    def __init__(self, spin=True, scissor=0, **kwargs):
        super().__init__(**kwargs)
        # self.Efermi = Efermi
        self.kwargs_formula.update(dict(spin=spin, Efermi=kwargs['Efermi'], scissor=scissor))
...
class Mag_sus_inter_correction_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        super().__init__(data_K, **parameters)
        M = data_K.Q2.magnetic_dipole_inter_correction
        Edif = data_K.Q2.Edif
        invEdif_eta = Edif / (Edif ** 2 + data_K.Q2.eta ** 2)

        summ = np.zeros((data_K.nk, data_K.num_wann, data_K.num_wann, 3, 3), dtype=complex)
        summ += -2 * np.einsum('knmi, knml, knm -> knmil', M, M.conj(), invEdif_eta)
...
class Mag_sus_inter_correction(DynamicCalculator):
    def __init__(self, spin=True, **kwargs):
        super().__init__(**kwargs)
        self.kwargs_formula.update(dict(spin=spin))
        self.Formula = Mag_sus_inter_correction_formula
        self.constant_factor =  factors.factor_cell_volume_to_m
        self.transformTR = transform_ident
        self.transformInv = transform_ident
        self.sign = 1
...
class Mag_sus_occ_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        ...
        self.Imn = np.real(summ) * anti_kron[...,None,None]
        self.ndim = 2
...
class Mag_sus_geo_wang_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        ...
        Edif = data_K.Q2.Edif
        M = data_K.Q2.magnetic_dipole_no_ring
        O = data_K.Q2.berry_curvature_wang_abelian
...
class Mag_sus_occ_mass_no_ring_formula(Formula):
    def __init__(self, data_K, spin=True, **parameters):
        super().__init__(data_K, **parameters)
        ...
        summ = np.zeros((data_K.nk, data_K.num_wann, data_K.num_wann, 3, 3), dtype=complex)
        summ += -1/4 * elementary_charge**2 * hbar**(-2) * speed_of_light**(-2) * np.einsum('knndb, knma, kmnd, iab, lcd -> knmil', mass_sum_rule, A, A, lev, lev)
...
class Mag_sus_occ_mass_no_ring(DynamicCalculator):
    def __init__(self, spin=True, **kwargs):
        super().__init__(**kwargs)
        self.kwargs_formula.update(dict(spin=spin))
        self.Formula = Mag_sus_occ_mass_no_ring_formula
        self.constant_factor =  factors.factor_cell_volume_to_m
        self.transformTR = transform_ident
        self.transformInv = transform_ident
        self.sign = 1
